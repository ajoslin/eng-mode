...
Eng Mode never starts on its own: the `eng-mode` skill is hidden from model invocation, so ordinary requests stay on your default (cheaper) model with the expert lens off. Run `/eng-mode [request]` to enter it. The command switches the session to the `eng_mode` model role (set `modelRoles.eng_mode` in `config.yml`, e.g. `cliproxy/claude-fable-5-1:low`), marks the session as in Eng Mode so the expert lens classifies prompts, and injects the Eng Mode skill as a user invocation. If the role is unset or unavailable, the command warns and keeps the current model. Eng-Advisor is off by default in every session and only runs after an explicit `/eng-advisor on`.
