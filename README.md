...
Restart OMP. Plugin load also installs an allowlisted judgment-layer overlay at `~/.agents/skills` (symlinks into the plugin package) so Codex and OpenCode can load those skills without cloning this repo. Then open every repository where you use Eng Mode and run:
...
- `eng-agent-skills.ts` idempotently exposes the judgment-layer `agentSkillsAllowlist` at `~/.agents/skills/<name>` as directory symlinks into the installed plugin. Codex and OpenCode scan that home path; OMP still prefers the `omp-plugins` copy. The overlay never writes a project's `.agents/skills` and never clones this repo into another worktree.
