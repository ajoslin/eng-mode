...
The modifier applies only when the prompt begins with `/eng-mode` and `/easy` stands alone as a leading or trailing word. `/easy-mode`, embedded `/easy`, and prompts that do not start with `/eng-mode` are unchanged. A missing or unavailable role renders an error and does not pin the session. Relaunch before recipes that require the expert-tier launch model.
