...
10. Invoke `{ action: "status", store: <store> }`. Confirm the returned JSON contains `units`, `ledger`, `gates` including the resolved gate, `frontier`, `summary`, and `changed`. `status` also writes `status.md` in the store. That Markdown holds the tables but has no `changed` field. `status` takes the store lock like other writes.

## Gotchas

Never point this recipe at a production store or repository. Do not disclose credentials or production identifiers in evidence. Steps outside frontier verification use invented disposable PR and SHA values. Step 8 requires actual identifiers from its dedicated non-production `gh stack`. After capturing evidence, stop the managed process and delete only the throwaway store and disposable profile. If step 8 used dedicated non-production resources, remove only the disposable stack and repository that this run created.

Store writes serialize per handle, and closing a handle drains accepted writes. Live lock takeover is not supported. A legacy flat `.orch.lock` file or unknown lock ownership blocks `init`, `status`, and every mutating action; read-only actions (`unit_get`, `unit_list`, `unit_counts`, `ledger_check`, `ledger_summary`, `inbox_peek`, `inbox_count`, `gate_list`, `frontier_show`, `standing_show`) never take the lock and keep working. Stop all writers and verify that none remain before manually removing that lock. New-format locks recover only when their recorded owner process is dead. Unit states and ledger rows are bookkeeping, not authorization to merge.

Bulk inbox recovery (`inbox_reclaim` without `claim`) reports preserved malformed or conflicting claims in `skipped`. Confirm those IDs and reasons render, valid owned pointers return to the inbox, and preserved claim files remain unchanged. Claims owned by another spawner are silently left in place, not listed in `skipped`; naming such a claim explicitly errors instead.
