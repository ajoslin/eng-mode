...
6. Invoke `complete`, then `get`. Confirm both render `Status: complete`. The `complete` result also appends a `Goal achieved. Report final budget usage to the user: ...` line.
7. Invoke `drop`. Confirm it renders `Status: dropped`. Invoke `get` and confirm `No active goal.`.

## Gotchas

The wrapper delegates to OMP's same-name native tool; every rendered string above is native OMP output, so an OMP upgrade can change it without any change in this repository. The wrapper's own schema requires a create budget of at least `500000000`, and `execute` clamps a smaller value up to that minimum rather than rejecting it; the native tool accepts any positive integer. Goal state owns the objective and accounting, not repeated turns. Token counts after create grow with each turn. `resume` on an already-active goal is a no-op that keeps `Status: active`. `drop` after `complete` still renders `Status: dropped`. Always drop the disposable goal before removing the profile.
