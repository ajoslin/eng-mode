...
Spawn all explorers in one `task` batch as `scout` agents. `scout` supplies the read-only tool boundary; model routing comes from the agent definition and OMP configuration, never task payload fields.
...
Spawn one `scout` agent that explores and explains in one pass.
...
Once all explorers return, spawn one `scout` agent to synthesize their findings into one coherent explanation.
...
After the explanation is complete, dispatch the four pinned panel agents (`panel-opus`, `panel-sol`, `panel-fable`, `panel-grok`) in one `task` batch. Their project agent definitions own model-family routing and read-only tools. If a seat is unavailable, report the dropout; never silently substitute a duplicate family.
