...
3. Pick a writable agent type and assign each candidate exclusive ownership of a distinct `local://arena-<slug>-candidate-<n>.md` artifact. For UI-flavored artifacts (layout, interaction, density, visual direction, or UI prototype variants), candidates default to `design-agent`, which routes through the `designer` role; mix in another writer only when it supplies needed structural diversity. Competing candidates never use `isolated: true`. OMP applies isolated writer output to the parent tree, while arena outputs are alternatives whose union is not intended. Read-only panel agents cannot be candidates.
...
1. one synthesized, integrated, verified artifact.
