...
4. Before lead-owned exploration, choose exactly one path. Use `scout` when a report is sufficient. Otherwise open `checkpoint` before the first read, grep, glob, LSP, log, or verification call, then `rewind` with the findings before editing or yielding.
