...
1. Read `principle-guard-the-context-window` in every session. Before lead-owned exploration, choose exactly one path. Use `scout` when a report is sufficient. Otherwise open `checkpoint` before the first read, grep, glob, LSP, log, or verification call, then `rewind` with the findings before editing or yielding.
...
- Delegation: `principle-guard-the-context-window` is mandatory at Start. `principle-never-block-on-the-human` applies when tempted to ask about reversible work that evidence can settle.
