...
- The lead may edit. The extension trims lead `edit`/`write`/`todo` results to a one-line confirmation plus an `artifact://` pointer, and caps oversized lead tool results to a head, a tail, and an `artifact://` pointer. `/eng-lead-writes block` is the opt-in strict gate. Identical re-reads are left to the harness (`compaction.supersedeReads` plus the repeat-read hint). A long fix-retry loop still belongs in `implementation-agent`, `judgment-agent`, or `sonic` with a complete brief: a fresh worker runs the loop on a small context; the lead running it replays its whole history on every tool call.
