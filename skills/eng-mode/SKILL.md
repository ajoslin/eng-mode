...
5. Read the project's `project-standards` contract for repository law, domain vocabulary sources, and the repository-selected specialized skills. Read `CONTEXT.md` for domain vocabulary and nearby ADRs for settled decisions when the project keeps them. `CONTEXT.md` is never a spec.
...
- Use the most specific OMP agent type, cheapest first. `sonic` for mechanical edits with no judgment (renames, moves, generated files, applying a stated diff). `scout` for read-only exploration with a bounded question. `implementation-agent` is the default writer for any brief that names files, symbols, the data shape, and observable success. `judgment-agent` only when the brief states which criterion applies: the scope is vague, the change crosses subsystems, it touches concurrency, or the algorithm is subtle; write that criterion in the brief. `reviewer` and `security-reviewer` review. A brief that qualifies for `sonic` or `implementation-agent` but is sent to `judgment-agent` is a routing defect. Agent definitions own model routing.
- The lead does not edit repository files; the extension blocks lead `write` and `edit` on repository paths (`/eng-lead-writes allow` overrides for one session). Every edit, fix-retry loop, patch regeneration, dependency reinstall, or test-fix cycle goes to `implementation-agent`, `judgment-agent`, or `sonic` with a complete brief. A fresh worker runs the loop on a small context; the lead running it replays its whole history on every tool call. The extension also blocks a lead re-read of an unchanged path (append `?fresh` to force) and caps oversized lead tool results to a head, a tail, and an `artifact://` pointer; read the artifact by range instead of re-running the tool.
...
- Delegation: `principle-guard-the-context-window` applies when large output, repeated reads, fan-out planning, or lead-owned exploration creates wider context-budget decisions. Its scout versus checkpoint rule is mandatory at Start. `principle-never-block-on-the-human` applies when tempted to ask about reversible work that evidence can settle.
