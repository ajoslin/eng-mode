...
Read the **Principles** section of the `eng-mode` skill for applicability. Then read only the sibling `principle-*` leaves that govern a plan decision. Cross-link those leaves. Do not add a first todo to read the full index.
