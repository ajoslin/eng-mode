...
4. Verify each component against its baseline via image diff: for mapped web surfaces, the parent runs the project verification contract's workflow (`verify-project`); other surfaces use their real screenshot driver. For sustained convergence, use a native `/goal`. Sleep only if the operator asked for an interval. Eng Mode owns the frozen image-diff metric and stops at zero, plateau, or the declared bound. Never alter the baseline or ruler; a necessary ruler change ends the current run and invalidates prior comparisons.
