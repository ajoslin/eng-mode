...
3. Make only task-owned work durable. Inventory paths changed by this task, stage only those paths, then `git commit -m "WIP checkpoint"`. Leave unrelated dirty state unstaged and record it in the resume note. If task-owned changes cannot be isolated safely or are known broken, write the note without creating a mixed or broken commit.
...
5. Leave `goal` incomplete. If `loop` is active, pause it after the checkpoint is durable.

**Reply:** the resume-note path, where you are in the loop, what's on disk versus still in your head, commits made, tree state, durable goal state, loop pause evidence, and the first action on resume. This is a pause, not a final report. Resume is the Session pickup playbook reading the named note.
