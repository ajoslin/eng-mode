...
2. Dispatch **one** `task` batch of two seats, same frozen SHA, same diff: `pre-pr-swarm`, `meaningful-contribution`. No serial order. No remediating between seats. No harsh-rerun loop. If a seat drops, continue and record the dropout.
