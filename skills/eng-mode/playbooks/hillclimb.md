...
3. Use a native `/goal` for unattended persistence. Require an explicit iteration or duration bound. The playbook owns the metric and stop policy. Sleep only if the operator asked for an interval. The target includes both desired improvement and any minimum-attempt floor.
...
8. After each measurement, the lead checks target, plateau, and bound. At target, complete the native goal. At plateau, pivot mechanisms without weakening the ruler; at the declared bound, stop and report. Never continue through an unbounded handwritten loop.
