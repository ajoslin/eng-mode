...
4. The root runs **Opening a PR**, appends the verified PR through **graphite** (`gt track -p <parent>` or `gt --no-interactive create`, then `gt submit --no-interactive`), and runs Babysit. A rewritten head loses its verdict unless patch identity is unchanged; corrections use a fresh writer followed by re-verification.
