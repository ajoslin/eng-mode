...
4. The root opens the verified PR through **github-stack** (`gh stack init` or `gh stack add`, then `gh stack submit --auto`), and runs Babysit. A rewritten head loses its verdict unless patch identity is unchanged; corrections use a fresh writer followed by re-verification.
