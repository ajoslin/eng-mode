...
**Build and verify one dependent PR stack; never land it.**
...
4. The root opens the verified PR, appends it to the stack through the project's stack tooling, and runs Babysit. A rewritten head loses its verdict unless patch identity is unchanged; corrections use a fresh writer followed by re-verification.
