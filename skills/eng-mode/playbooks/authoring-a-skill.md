...
**Own the agent-facing voice.** When in doubt, delete. Keep only prose that changes a decision. Write direct imperatives. Explain a rule only when it would otherwise be ambiguous. Match the voice to the skill's scope.
...
2. Keep `SKILL.md` concise and progressively disclosed. Move substantial detail into one-level references. Link to structural sources such as types, configuration, and canonical READMEs instead of restating them. Show every shipped script's invocation in the body.
