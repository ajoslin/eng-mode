...
3. For a changed module interface, run `architect`. For an empirical fork, run `prototype`.
4. Set the throughput checkpoint with four items: blocking first steps; independent workstreams; shared mutable state; smallest safe decomposition. Keep coherent work local; delegate only independent slices.
