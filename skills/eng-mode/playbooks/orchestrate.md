...
Create `.audit/orchestrate/<project-slug>/` (gitignored). That path is the `store` argument of every `eng_orch` call. Choose a stable coordinator identifier, record it in the first standing order, and pass it as `spawner` to `init`, `inbox_push`, `inbox_claim`, `inbox_ack`, and `inbox_reclaim`. Every worker brief receives that literal identifier. `init` returns the store path, not a session id. Ownership is cooperative, not authenticated. The tool partitions claims by the supplied spawner but does not restrict canonical mutations by agent identity. Workers and verifiers publish their own completion pointers with `inbox_push`. The coordinator alone claims and acknowledges pointers, appends `overview.md`, and applies canonical mutations at drain points. Other sessions and agents are readers.
...
- `ledger.tsv` is the accepted-verdict index keyed by PR and head SHA. Recording the same key replaces its previous verdict. Preserve historical proof in the immutable receipts referenced by each accepted decision.
...
- `inbox_reclaim` returns recovered `claims`, their `pointers`, and `skipped` claim IDs with reasons. Inspect `skipped` before declaring recovery complete. Malformed or conflicting claims remain on disk for repair; valid claims can still recover. Ordinary claims owned by another coordinator are left alone.
