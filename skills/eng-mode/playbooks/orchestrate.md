...
- `frontier.json` is the computed merge frontier, per Stack safety. Recompute from `gt`, never from narrative.
...
FORBIDDEN    no rebase, no force-push, no gt, no isolated PR owners, no fixes outside scope, plus unit-specific bans
...
- Stacks exist after `gt submit --no-interactive`. `gh pr create --base` is not a stack. Never GitHub auto-merge on a stacked PR.
