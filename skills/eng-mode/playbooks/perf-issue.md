...
1. Capture a baseline on the matching surface: the project verification contract (`verify-project`) for mapped web behavior, OMP `browser` for other web UI, `debug` or the relevant profiler for runtime state, and the actual managed process for CLI/TUI. Preserve the trace or profile artifact.
