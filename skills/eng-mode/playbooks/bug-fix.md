...
2. Trace the mechanism before proposing fixes. Use `how`, `why`, ADRs, LSP, and `debug`. A stubborn falsifiable search is a `/goal` with sleep between iterations: `/goal do x until y, sleep 30 minutes between iterations` (or whatever interval). Require an explicit bound. Never edit the ruler.
