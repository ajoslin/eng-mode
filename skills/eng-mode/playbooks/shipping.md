...
7. Once landing starts, do not mutate topology. Requested watching continues under a native goal or an operator-started bounded `/loop`; never hand-roll polling. Each tick re-checks inventoried review-agent checks on the current SHA before merge, and the playbook—not OMP—decides when the gate is satisfied.
