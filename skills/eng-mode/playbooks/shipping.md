...
1. Use the **github-stack** skill and the GitHub workflow skill `project-standards` names. Do not use Graphite.
...
6. Use `github-stack` to sync, submit, or merge (`gh stack sync`, `gh stack submit --auto`, `gh stack merge <target> --yes`). Never `gh pr merge` a stacked PR. Never raw-auto-merge child PRs. If the base uses a merge queue, proceed only when `gh stack merge` proves ordered landing; otherwise stop and report the gate.
