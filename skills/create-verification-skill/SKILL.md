...
Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates that as the project's native verification contract (`.agents/skills/verify-project/`) tailored to the repo. You write the generator's output for the next agent, not for a human. It will be read cold, mid-task, by an agent that has never seen the app.
...
Write `.agents/skills/verify-project/SKILL.md` with YAML frontmatter (`name: verify-project` and a `description` that names the app, the surface, and when to reach for it). Without frontmatter the skill never registers. Include these sections, each grounded in what the interview actually found (no placeholders left):
