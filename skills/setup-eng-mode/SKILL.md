...
1. **Prepare:** atomically add and validate all new roles while retaining all old roles: `code` from `boja_code` when absent, `judgment` from `boja_judgment`, `adversary` from `boja_adversary`, `fast` from `boja_fast`. Prepare `panel_sol`, `panel_fable`, and `panel_deepseek` from the workstation's current pinned panel selectors. An existing valid new value wins. A differing old/new pair is a conflict and produces no change without explicit user selection. On a workstation with no old roles, those absent panel roles require explicit selection from actually available models. Never invent a concrete model. Leave any workstation `panel_opus` role unchanged; the shipped Opus seat uses the package-owned selector instead.
...
1. Verify each shipped agent (`implementation-agent`, `judgment-agent`, `comment-sicko`, `panel-opus`, `panel-sol`, `panel-fable`, `panel-deepseek`) is discoverable and every model chain resolves end to end. The workstation config supplies concrete models through roles, except for `panel-opus`. Its package-owned primary is `cliproxy/claude-opus-5-5:low`, followed by `@adversary` and `@review`. The extension registers this static model at `http://100.73.208.98:8317/v1` without credentials. OMP's `N/A` keyless sentinel satisfies its registration API without sending an Authorization header. Do not remap workstation roles to install this pin.
...
Restart OMP after installing or updating the extension, then verify `eng_orch contracts` and a throwaway store initialization through the registered tool.
