...
1. Capture the live signal on the matching surface: the project verification contract's own workflow (`verify-project`) for a mapped web trace, OMP `browser` for other web surfaces, `debug` or the relevant profiler for a spinning process or leak, and the actual managed process for CLI/TUI. Preserve a CPU profile, heap snapshot, or trace—a real artifact, not a guess.
...
3. Prove the mechanism before believing it. When the verification contract owns a private browser, instrument through evaluation or a CDP session created inside the owning harness; external OMP `browser` cannot attach to that private Chromium. For other live processes, use the matching debugger or runtime instrumentation. A plausible-but-unconfirmed cause can be wrong while the real one sits one layer over.
