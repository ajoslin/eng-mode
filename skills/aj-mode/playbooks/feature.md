...
3. For a changed module interface, run `architect`; otherwise record `architect skipped: <reason>`. For an empirical fork, run `prototype`.
...
6. Prove user-visible behavior on the actual surface. Mapped web work uses the project verification contract (`verify-project`); failed health gate, unmapped path, shared-stack, or wrong-surface evidence is `INCONCLUSIVE`. Add a permanent test only for an uncovered observable contract.
