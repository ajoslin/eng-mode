...
2. Trace the mechanism before proposing fixes. Use `how`, `why`, ADRs, LSP, and `debug`. Use `/loop` only for a stubborn falsifiable search.
...
5. Verify the original reproduction on the same surface. Mapped web proof again uses `verify-project`; failed health gate, unmapped path, shared-stack, or wrong-surface evidence is `INCONCLUSIVE`. Then run narrow applicable checks.
