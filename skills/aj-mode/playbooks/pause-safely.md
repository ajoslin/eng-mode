...
3. Make only task-owned work durable. Inventory paths changed by this task and commit only those paths in one clear `wip:` commit. Leave unrelated dirty state untouched and record it in the resume note. If task-owned changes cannot be isolated safely or are known broken, write the note without creating a mixed or broken commit.
