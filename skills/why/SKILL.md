...
Before spawning investigators, list the available MCPs from the OMP environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory OMP exposes for enabled MCP servers.
...
Dispatch each investigator as a `scout` when repository and web evidence are sufficient. When a category requires an MCP not exposed to `scout`, use the most specific tool-capable project agent and prohibit writes in its brief. Agent definitions and OMP configuration own model routing; never pass Cursor-style `model` or `readonly` task fields.
...
Spawn one synthesizer using the most specific tool-capable reviewer available. Its brief permits citation spot-checks but prohibits writes. Model routing comes from the agent definition and OMP configuration.
