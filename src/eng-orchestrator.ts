...
export async function executeEngOrch(input: Input): Promise<unknown> {
  if (input.action === "contracts") {
    ...
    return decideRepositoryContracts({
      repositoryRoot,
      mode: input.mode ?? "code-producing",
      observations,
    });
  }
  const store = openStore(resolveStore(input));
  try {
    switch (input.action) {
      case "init":
        return await store.init({
          spawner: required(input.spawner, "spawner"),
        });
      case "unit_add":
        return await store.units.add({
          id: required(input.id, "id"),
          track: required(input.track, "track"),
          ...(input.brief === undefined ? {} : { brief: input.brief }),
        });
      case "unit_set":
        return await store.units.set({
          id: required(input.id, "id"),
      ...
      case "unit_get":
        return await store.units.get(required(input.id, "id"));
      case "unit_list":
        return await store.units.list({
          ...(input.state === undefined ? {} : { state: input.state }),
          ...(input.track === undefined ? {} : { track: input.track }),
        });
      case "unit_counts":
        return await store.units.counts();
      case "ledger_record":
        return await store.ledger.record({
          pr: input.pr ?? 0,
          sha: required(input.sha, "sha"),
          verdict: parseVerdict(required(input.verdict, "verdict")),
          evidence: required(input.evidence, "evidence"),
          ...(input.verifier === undefined ? {} : { verifier: input.verifier }),
        });
      case "ledger_check":
        return await store.ledger.check({
          pr: input.pr ?? 0,
          sha: required(input.sha, "sha"),
        });
      case "ledger_summary":
        return await store.ledger.summary();
      case "inbox_push":
        return await store.inbox.push({
          spawner: required(input.spawner, "spawner"),
      ...
      case "inbox_claim":
        return await store.inbox.claim({
          spawner: required(input.spawner, "spawner"),
        });
      case "inbox_ack":
        return await store.inbox.ack({
          spawner: required(input.spawner, "spawner"),
          claim: required(input.claim, "claim"),
        });
      case "inbox_reclaim":
        return await store.inbox.reclaim({
          spawner: required(input.spawner, "spawner"),
          ...(input.claim === undefined ? {} : { claim: input.claim }),
        });
      case "inbox_peek":
        return await store.inbox.peek();
      case "inbox_count":
        return await store.inbox.count();
      case "gate_park":
        return await store.gates.park({
          id: required(input.id, "id"),
          question: required(input.question, "question"),
          options: required(input.options, "options"),
          defaultAnswer: required(input.defaultAnswer, "defaultAnswer"),
        });
      case "gate_list":
        return await store.gates.list();
      case "gate_resolve":
        return await store.gates.resolve({
          id: required(input.id, "id"),
          answer: required(input.answer, "answer"),
        });
      case "frontier_set":
        return await store.frontier.set({
          repo: required(input.repo, "repo"),
          ...(input.prs === undefined ? {} : { prs: input.prs }),
        });
      case "frontier_show":
        return await store.frontier.show();
      case "standing_add":
        return await store.standing.add({ line: required(input.line, "line") });
      case "standing_show":
        return await store.standing.show();
      case "status":
        return await store.status.render();
    }
...
export function registerEngOrchestration(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    description:
      "Validate repository contracts or update Eng Mode's durable orchestration store.",
    parameters: z.object({
